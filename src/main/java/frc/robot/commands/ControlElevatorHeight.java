...
public class ControlElevatorHeight extends CommandBase {
  ...
  public void execute() {
    ...
    if(mElevator.getElevatorEncoderDistance() >= (80*ElevatorSensorConstants.kTicksPerInch)){
      mSecondaryController.setRumble(RumbleType.kLeftRumble, .35);
      mSecondaryController.setRumble(RumbleType.kRightRumble, .35);
    }
    else{
      mSecondaryController.setRumble(RumbleType.kLeftRumble, 0);
      mSecondaryController.setRumble(RumbleType.kRightRumble, 0);
    }
    

    // if(mElevator.getElevatorEncoderDistance() < -20){
    //   mElevator.openLoopControl(Math.abs(yAxisRight));
    // }
    // else{
    //   mElevator.openLoopControl(yAxisRight);
    // }
